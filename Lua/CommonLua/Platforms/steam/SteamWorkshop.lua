...
local GatherSteamModsDownloadList = function(mods, filter)
  local steam_ids = {}
...
function OnMsg.GatherModDownloadCode(codes)
  local steam_ids = GatherSteamModsDownloadList(ModsLoaded, function(mod)
    return mod.source == "steam"
  end)
  if not next(steam_ids) then
    return
  end
  codes.steam = string.format("DebugDownloadSteamMods%s", TableToLuaCode(steam_ids, " "))
end
function OnMsg.DebugDownloadExternalMods(mods, out_threads)
...
function OnMsg.DebugCopyExternalMods()
  DebugCopySteamMods()
end
