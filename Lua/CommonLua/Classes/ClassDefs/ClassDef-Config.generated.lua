DefineClass.Achievement = {
  ...
  properties = {
    ...
    },
    {
      ...
      default = false,
      translate = true,
      context = "(limited to 100 characters on XBOX)"
    },
    {
      id = "how_to",
      name = "How To",
...
function Achievement:GetError(platform)
  ...
  if ShouldTestPlatform("xbox_one") or ShouldTestPlatform("xbox_series") then
    if #TDevModeGetEnglishText(self.description) > 100 then
      errors[#errors + 1] = string.format("XBOX achievement description must be limited to 100 characters!")
    end
    if 100 < #TDevModeGetEnglishText(self.how_to) then
...
DefineClass.VideoDef = {
  ...
  properties = {
    ...
    },
    {
      ...
      editor = "text",
      default = "-c:v libx265 -tag:v hvc1 -preset veryslow -pix_fmt yuv420p",
      no_edit = function(self)
        return not self.present_xbox_series
      end
    },
