...
function GedOpEditMod(socket, obj, item_idx)
  ...
  if not CanLoadUnpackedMods() then
    ModLog(true, T({
      970080750583,
      "Error opening <ModLabel> for editing: cannot open unpacked mods",
      mod
    }))
    return
  end
...
function GedOpNewModItem(socket, root, class_or_instance)
  ...
  mod:SortItems()
  UpdateParentTable(item, mod.items)
...
function GedOpDuplicateModItem(socket, root, path)
  ...
  UpdateParentTable(new_item, parent)
  ...
  if IsKindOf(new_item, "ModItemCode") then
    ...
    if err then
      ModLogF(true, "Error duplicating mod code: %s", err)
    end
  end
...
function GedOpDeleteModItem(socket, root, path, force)
  ...
  if 2 < #path then
    ...
    return selection, function()
      ...
      if err then
        ModLogF(true, "Error during delete undo: %s", err)
...
function GedOpGenTTableMod(socket, root)
  GedOpSaveMod(socket, root)
  local modDef = root[1]
  local csv = {}
  for _, item in ipairs(modDef and modDef.items or {}) do
      ...
      for _, propMeta in ipairs(obj.GetProperties and obj:GetProperties()) do
        ...
        if propVal ~= "" and IsT(propVal) then
          ...
          if getmetatable(propVal) == TConcatMeta then
            ...
            csv[#csv + 1] = {
              id = TGetID(propVal),
              text = TDevModeGetEnglishText(propVal),
              context = context,
              voice = voice
            }
          end
