...
function FlightMarkBetween(ptFrom, ptTo, min_height, obj_radius)
  ...
  local now = GameTime()
  if not (fbox and FlightMap and IsValidPos(FlightMarkFrom) and IsValidPos(FlightMarkTo)) or FlightPassVersion ~= PassVersion or now - FlightTimestamp > FlightMapLifetime or FlightMarkMinHeight ~= min_height or FlightMarkObjRadius ~= obj_radius or Intersect2D(boxdiag(FlightMarkFrom, FlightMarkTo), boxdiag(ptFrom, ptTo)) ~= irInside then
...
function OnMsg.OnPassabilityChanged(box)
  for _, obj in ipairs(FlyingObjs) do
    local flight_path = obj.flight_path
    if flight_path and FlightPathIntersectEst(flight_path, box) then
      obj.flight_path = nil
    end
  end
end
...
function FlightFindReachableLanding(pos, pfclass, mode)
  local flight_map = FlightMap
  if not flight_map then
    return
  end
  if FlightIsLandingPos(pos, flight_map, pfclass) then
    return pos
  end
  local takeoff = mode == "takeoff"
  local filter
  if takeoff then
    function filter(x, y, z, pos, pfclass)
      return ConnectivityCheck(pos, x, y, z, pfclass)
    end
  else
    filter = ConnectivityCheck
  end
  local x, y, z = FlightFindLanding(flight_map, pos, pfclass, filter, pos, pfclass)
  if not x then
    return
  end
  ...
  local path = pf.GetPosPath(src, dst, pfclass)
  if path then
    local i1, i2, di
    if takeoff then
      i1, i2, di = #path - 1, 2, -1
    ...
    for i = i1, i2, di do
      ...
      if pt:IsValid() then
        if not FlightIsLandingPos(pt, flight_map, pfclass) then
          pt = terrain.FindReachable(pt, const.tfrPassClass, pfclass, const.tfrLimitDist, const.FlightTile, 1, const.tfrLuaFilter, FlightIsLandingPos, flight_map, pfclass)
        end
        if pt then
          landing = pt
          break
