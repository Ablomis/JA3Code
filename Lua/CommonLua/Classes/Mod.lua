...
local ModResourceExists = function(path, item)
  if io.exists(path) then
    return true
  end
  if item and IsKindOf(item, "Sample") then
  ...
  for i, prop in ipairs(item:GetProperties()) do
    if (prop.editor == "browse" or prop.editor == "ui_image") and not prop.os_path then
      ...
      if (path or "") ~= "" and not path:starts_with(ModContentPath) and not ModResourceExists(path, item) then
        ...
        elseif not string.find(path, EscapeMagicSymbols(mod_os_path)) then
...
function ModDef:LoadItems()
  ...
  if not ok then
    local err = items
    ModLogF(true, "Failed to load mod items from %s. Error: %s", self:GetModLabel(), err)
  else
...
function ModDef:GetModLabel()
  return Untranslated(string.format("%s (id %s, v%s)", self.title, self.id, self:GetVersionString()))
end
  ...
  return function()
    ...
    if mod_def then
      if not mod_def.items then
        ModLogF(true, "This savegame tries to load Mod %s, which is present, but not loaded", mod_def:GetModLabel())
      elseif mod_def:CompareVersion(mod_info) ~= 0 then
        ModLogF(true, "This savegame tries to load Mod %s, which is loaded with a different version %s", mod_info:GetModLabel(), mod_def:GetVersionString())
      else
        ModLogF("This savegame loads Mod %s", mod_def:GetModLabel())
      end
      return mod_def
    else
      ModLogF(true, "Savegame references Mod %s which is not present", mod_info:GetModLabel())
    end
    return mod_info
  end
end
...
function OnMsg.BugReportStart(print_func)
  ...
  for i, mod in ipairs(ModsLoaded) do
    table.insert(list, string.format("%s (id %s, v%s, source %s, required Lua: %d, saved with Lua: %d", mod.title, mod.id, mod:GetVersionString(), mod.source, mod.lua_revision, mod.saved_with_revision))
  end
  table.sort(list)
  ...
  for i, folder in ipairs(folders) do
    ...
    if env and IsKindOf(def, "ModDef") then
      ...
      if def:IsTooOld() then
        ModLogF("Outdated definition for %s loaded from %s. (Unsupported game version)", def:GetModLabel(), def.source)
      end
      local old = new_mods[def.id]
      if old then
        if 0 <= old:CompareVersion(def) then
          ModLogF("Mod %s ignored because a newer version is already loaded", def:GetModLabel())
        else
          ModLogF("Mod %s will replace an older version of the same mod", def:GetModLabel())
          new_mods[def.id] = def
          mod_used = true
        end
      else
        ...
        if dep_mod then
          ...
          if err then
            if err == "cycle" then
              ...
              ModLogF("Mod %s creates circular dependency cylce with %s.", mod:GetModLabel(), other_mods)
            elseif err == "not loaded" then
              ModLogF("Cannot load %s because dependency mod %s is not active.", mod:GetModLabel(), dep_mod:GetModLabel())
            elseif err == "incompatible" then
              ModLogF("Cannot load %s because dependency mod %s is not compatible.", mod:GetModLabel(), dep_mod:GetModLabel())
            end
            stack[mod.id] = nil
            return err
          end
        else
          ModLogF("Cannot load %s because dependency mod %s is not found.", mod:GetModLabel(), dep.title)
          stack[mod.id] = nil
          return "not found"
        end
      end
    ...
    for i, id in ipairs(queue) do
      ...
      if mod then
        if not mod:IsTooOld() or mod.force_reload then
          ...
          ModLogF("Outdated mod %s cannot be loaded. (Unsupported game version)", mod:GetModLabel())
        end
      end
    end
  end
...
function ModsLoadCode()
  for _, mod in ipairs(ModsLoaded or empty_table) do
    ...
    for _, filename in ipairs(mod.code or empty_table) do
      ...
      if not ok then
        ModLogF(true, "Error loading %s: %s", file_path, err or "Unknown error")
      end
    end
...
function DebugDownloadExternalMods(mods, out_threads)
  out_threads = out_threads or {}
  Msg("DebugDownloadExternalMods", mods, out_threads)
end
function DebugCopyExternalMods()
  Msg("DebugCopyExternalMods")
  ModsReloadDefs()
end
function DebugDownloadSavegameMods()
  if not IsRealTimeThread() then
    CreateRealTimeThread(DebugDownloadSavegameMods)
    return
  end
  local threads = {}
  DebugDownloadExternalMods(SavegameMeta.active_mods, threads)
...
function ModItemOptionToggle:GetEditorView()
  return string.format("Option %s = %s <color 0 128 0> %s", self.name, self.DefaultValue and "On" or "Off", self.comment)
end
...
function ReportModLuaError(mod, err, stack)
  ...
  mod_print("Lua error in mod %s (id %s, v%s) from %s", mod.title, mod.id, ver_str, mod.source)
