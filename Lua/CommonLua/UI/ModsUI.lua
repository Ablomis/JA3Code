if not config.DisableLegacyModsUI then
  return
end
if FirstLoad then
  g_InitialMods = false
...
function ModsUIUninstallLocalMod(mod, quiet)
  ...
  if g_ModsUIContextObj then
    ...
    g_ModsUIContextObj:GetMods(mod)
  end
  ObjModified(mod)
end
...
function ModsUIObject:Init()
  ...
  if IsModsBackendLoaded() and not g_ModsBackendObj:AttemptingLogin() then
    self:GetMods()
  else
  end
end
...
function InstalledModsUIObject:GetMods(modify_obj, skip_install)
  DeleteThread(self.get_mods_thread)
  self.get_mods_thread = CreateRealTimeThread(function(self)
    self.last_browse_y = false
    self.last_browse_item = false
    ...
    for k, v in sorted_pairs(Mods) do
      ...
      if not seen[k] and table.array_isubset(tags, mod_tags) then
        ...
        local favorited
        if self.favorites and mod_ui_entry then
        ...
        if (not self.only_compatible or compatible) and (not self.favorites or favorited) then
...
function InstalledModsUIObject:SetInstalledSortMethod(id)
  if self.set_sort ~= id then
    self.set_sort = id
    if LocalStorage then
