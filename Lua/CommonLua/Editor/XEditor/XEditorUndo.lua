...
if FirstLoad then
  ...
  EditorUndoPreserveHandles = config.MapPatchTesting or false
end
...
function XEditorUndoQueue:RestoreObject(obj, obj_data, prev_data)
  ...
  for _, prop_meta in ipairs(obj:GetProperties()) do
    ...
    if value ~= nil then
      ...
      elseif not (not EditorUndoPreserveHandles or EditorPasteInProgress) or id ~= "Handle" then
...
function XEditorUndoQueue:BeginOp(settings)
  ...
  local root_op = self.op_stack[1] or edit_operation
  if not root_op.selection then
    root_op.selection = SelectionEditOp:new()
    for i, obj in ipairs(editor.GetSel()) do
      root_op.selection.before[i] = self:GetUndoRedoHandle(obj)
    end
  end
  for _, grid in ipairs(editor.GetGridNames()) do
    if settings[grid] and not root_op[grid] then
      root_op[grid] = GridEditOp:new({
        name = grid,
...
function XEditorUndoQueue:ObjectPropertyEdited(obj, prop_id, old_value)
  local last_op = self.undo_queue[self.undo_index]
  if not last_op or last_op.name ~= "Edit properties" then
    last_op = {
      name = "Edit properties",
      objects = ObjectsEditOp:new({
        data = {},
        by_handle = {}
      })
    }
    self:AddEditOp(last_op)
  end
  local obj_op = last_op.objects
  local handle = self:GetUndoRedoHandle(obj)
  local obj_data = table.find_value(obj_op.data, "__undo_handle", handle)
  if not obj_data then
    obj_data = {
      op = "update",
      __undo_handle = handle,
      after = {}
    }
    table.insert(obj_op.data, obj_data)
  end
  local prop_meta = obj:GetPropertyMetadata(prop_id)
  if obj_data[prop_id] == nil and obj_data.after[prop_id] == nil then
    if old_value == nil then
      old_value = obj:GetDefaultPropertyValue(prop_id)
    end
    obj_data[prop_id] = self:ProcessPropertyValue(obj, prop_id, prop_meta, old_value)
  end
  obj_data.after[prop_id] = self:ProcessPropertyValue(obj, prop_id, prop_meta, obj:GetProperty(prop_id))
end
...
function XEditorObjIdentifier(obj)
  return obj.class == "Collection" and obj.Index or xxhash(obj.class, obj:GetPos(), obj:GetAxis(), obj:GetAngle(), obj:GetScale())
end
function XEditorCreateMapPatch(filename)
  Msg("OnMapPatchBegin")
  XEditorUndo:RollToOpIndex(1)
  local hash_to_handle = {}
  for handle, obj in pairs(XEditorUndo.handle_to_obj) do
    if IsValid(obj) then
      hash_to_handle[XEditorObjIdentifier(obj)] = handle
    end
  end
  ...
  for idx, op in ipairs(XEditorUndo.undo_queue) do
    local affected = {}
  ...
  local obj_datas = edit_op.objects.data
  ...
  AsyncStringToFile(filename or "svnAssets/Bin/win32/Bin/map.patch", str)
  EditorUndoPreserveHandles = config.MapPatchTesting or false
  Msg("OnMapPatchEnd")
end
function XEditorApplyMapPatch(filename)
  ...
  XEditorUndo:UndoRedo("redo")
  table.remove(XEditorUndo.undo_queue)
  EditorUndoPreserveHandles = config.MapPatchTesting or false
  XEditorUndo.handle_remap = nil
  Msg("OnMapPatchEnd")
end
