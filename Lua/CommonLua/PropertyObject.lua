...
%s]], body, code)
  ...
  elseif editor == "ui_image" and value and value ~= "" and not self:IsDefaultPropertyValue(prop_meta.id, prop_meta, value) then
    ...
    if mod_def or not Platform.developer then
      local path = dir .. name
      local found_path
      if mod_def and ext and ext ~= "" then
        found_path = io.exists(path .. ext)
      else
        found_path = io.exists(path .. ".png") or io.exists(path .. ".tga") or io.exists(path .. ".dds")
      end
