...
function GedTToPropValue(value, default)
  ...
  if type(default) == "table" and #default == 2 and type(default[1]) == "number" then
...
function GedPropBrowse:Init(parent, context, prop_meta)
  ...
  if not prop_meta.read_only and (0 < #folders or prop_meta.os_path) then
    ...
    XTextButton:new({
      Id = "idButtonBrowse",
      Text = not (not self.in_mod_editor or prop_meta.os_path) and "Import" or "...",
...
function GedPropBrowse:OpenBrowseDialog(path, filter, exists, multiple, initial_file)
  ...
  if file_path then
    if self.in_mod_editor and not self.prop_meta.os_path then
      local mod_path = ConvertToOSPath(path)
    ...
    elseif not self.prop_meta.os_path then
      local dir, file, ext = SplitPath(file_path)
      file_path = dir .. file
    end
  end
