  ...
  properties = {
    {
      ...
      no_edit = Platform.xbox_one and not Platform.xbox_one_x and Platform.goldmaster,
    ...
    },
    {
      ...
      help_text = T(110177097630, "Smooths out jagged edges, reduces shimerring, and improves overall visual quality.")
    },
    {
    ...
    {
      name = T(956327389735, "Upscaling"),
      id = "Upscaling",
      category = "Video",
      storage = "local",
      editor = "choice",
      default = GetDefaultEngineOptions().Upscaling,
      no_edit = Platform.console and Platform.goldmaster,
      no_edit = function(self)
        return self.ResolutionPercent == "100"
      end,
    ...
    },
    {
      ...
      no_edit = Platform.console and Platform.goldmaster,
      help_text = T(540870423363, "Affects ths sharpness of the image")
    },
    {
    ...
    },
    {
      ...
      storage = "account",
      editor = "choice",
      default = GetDefaultAccountOptions().Language,
      no_edit = Platform.console,
    ...
    },
    {
      ...
      category = Platform.console and "Video" or "Display",
      storage = "local",
      editor = "number",
...
function OptionsObject:SetProperty(id, value)
  ...
  if OptionsData.VideoPresetsData[preset] and OptionsData.VideoPresetsData[preset][id] and PresetVideoOptions[id] and value ~= self:FindFirstSelectable(id, OptionsData.VideoPresetsData[preset][id]) then
...
function OptionsObject:SyncUpscaling()
  ...
  if not_selectable(uscaling_option) then
    local upscaling_index = table.findfirst(OptionsData.Options.Upscaling, function(idx, item)
      return not not_selectable(item)
    end)
  ...
  resolution_percent_option = resolution_percent_option or {
    hr = {
...
function OptionsObject:FindFirstSelectable(option, item_names)
  ...
  elseif is_selectable(option, item_names, self) then
    return item_names
  end
end
...
function SetupOptionRollover(rollover, options_obj, option)
  ...
  if help_text then
    ...
    help_title = help_title or prop_eval(option.name, option, options_obj)
  end
  if help_title then
