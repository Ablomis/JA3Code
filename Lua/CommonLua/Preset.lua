...
function Preset:SaveFiles(file_map, by_user_request, ged)
  ...
  if by_user_request and next(lua_source_failed_files) then
    ...
    ged:ShowMessage("Error Saving", string.format([[
Could not fetch a function's source code in file%s:

%s]], 1 < #files and "s" or "", files))
  end
  local to_delete = {}
    ...
    if 0 < #preset_list then
      ...
      if not errors then
        if Platform.developer and by_user_request and not self:IsKindOfClasses("MapDataPreset", "ConstDef") then
          ...
          dofile(path)
          g_PresetRefreshingFunctionValues = false
          CacheLuaSourceFile(path, file_data[false].code)
        end
        for _, preset in ipairs(preset_list) do
          preset:MarkClean()
        end
...
function QueueReloadAllPresets(file, change, force_reload)
  if Platform.ged or not force_reload and not Platform.developer then
    return
  end
  if file and not file:ends_with(".lua") then
    return
  end
...
function ReloadPresetsFromFiles()
  ...
  if 0 < #conflicted_classes then
    ...
    local q = [[
Preset data loaded from a file is about to overwrite changes made in the editor.
...
Continue?]]
  ...
  for name, func in pairs(compiled_lua_files) do
    PresetsLoadingFileName = name
    procall(func)
    CacheLuaSourceFile(name, source_lua_files[name])
  end
  PresetsLoadingFileName = false
