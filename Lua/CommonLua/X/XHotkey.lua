...
function KbdShortcut(virtual_key)
  ...
  local s = ""
  if virtual_key == const.vkControl or terminal.IsKeyPressed(const.vkControl) or Platform.osx and terminal.IsKeyPressed(const.vkLwin) then
    s = s .. "Ctrl-"
  end
...
function XInputShortcut(button, controller_id)
  ...
  if button == "LeftTrigger" or button == "RightTrigger" or button == "LeftShoulder" or button == "RightShoulder" then
    return s:sub(1, -2)
  end
  return s .. button
end
function MouseShortcut(button)
  ...
  local s = ""
  if terminal.IsKeyPressed(const.vkControl) or Platform.osx and terminal.IsKeyPressed(const.vkLwin) then
    s = s .. "Ctrl-"
  end
