...
function XTextEditor:ShouldProcessChar(ch)
  return (ch ~= "" and string.byte(ch) >= 32 or ch == "\r\n" or ch == "\n") and not IsControlPressed() == not IsAltPressed() and (not self.Filter or string.find(ch, self.Filter)) and not string.find(self.NegFilter, ch, 1, true) and (not self.AllowTabs or ch ~= "\t") and (self:GetMultiline() or ch ~= "\r\n" and ch ~= "\n")
end
...
function WaitControllerTextInput(default, title, description, max_length, password)
  if not HasControllerTextInput() then
    return default
  end
