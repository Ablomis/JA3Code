  ...
  properties = {
    {
      ...
      editor = "bool",
      default = true,
      help = "If true, map object animations are done in Lua time, which is read from hr.UILLuaTime."
    }
  },
  Clip = "self",
  UseClipBox = false,
...
function XMapObject:SetPos(posX, posY, time)
  ...
  self:AddInterpolation({
    ...
    flags = map.UseLuaTime and bor(const.intfInverse, const.intfLuaTime) or const.intfInverse,
...
function XMapObject:SetSize(width, height, time, hOrigin, vOrigin)
  ...
  self:AddInterpolation({
    ...
    flags = map.UseLuaTime and const.intfLuaTime
  })
  self:InvalidateLayout()
  self:InvalidateMeasure()
end
