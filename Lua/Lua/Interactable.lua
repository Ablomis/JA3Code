...
DefineClass.Interactable = {
  ...
  properties = {
    ...
    },
    {
      ...
      editor = "bool",
      default = true,
      help = "used by the DisableInteractionMarkerEffect to enable/disable interactables"
    }
  },
  highlight = true,
...
function InteractableVisibilityUpdate(units)
  ...
  for i, unit in ipairs(units) do
    if IsValid(unit) and unit:CanBeControlled(-1) then
      MapForEach(unit, lInteractableVisibilityRange, "Interactable", function(o)
        ...
        if can_interact_with == nil and unit:CanInteractWith(o) then
  ...
  if 0 < #__InteractableVisibility_CanNoLongerInteractWith then
  ...
  if 0 < #__InteractableVisibility_Targets then
