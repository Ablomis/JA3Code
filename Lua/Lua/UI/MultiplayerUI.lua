...
function MultiplayerFillGames(ui, filterType)
  ...
  for _, game in ipairs(available or empty_table) do
    ...
    if game_info and game[ADDRESS] ~= netGameAddress and game[VISIBLE] == "public" and game[PLAYERS] < game[MAX_PLAYERS] and 0 < game[PLAYERS] then
      local hostId = game_info.host_id
      if hostId and hostId ~= netAccountId then
...
function ShowMPLobbyError(context, err)
  ...
  elseif context == "platform-invite" then
    msg = CreateMessageBox(parent, T(634182240966, "Error"), T({
      960708199674,
      "(design)Could not open invitation dialog.",
      err = err
    }), T(325411474155, "OK"))
  ...
  elseif context == "disconnect-after-leave-game" then
    msg = CreateMessageBox(parent, T(634182240966, "Error"), T({
      763400292415,
      "Lost connection to multiplayer server. Reason: <MPError(err)>",
      err = err
    }), T(325411474155, "OK"))
...
function MultiplayerLobbySetUI(mode, param)
  ...
  if mode ~= "empty" and not netInGame then
    ...
    if err then
      return ShowMPLobbyError("connect", err)
    end
  end
  if mode ~= "empty" then
...
function GetGameInfo(game_address)
  if type(game_address) ~= "number" then
