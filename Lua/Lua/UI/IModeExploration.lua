...
local ShowQueuedMovementTargets = function()
...
function IsActivePauseAvailable()
  return IsGameRuleActive("ActivePause") and (not gv_UnitsBlockingPause or next(gv_UnitsBlockingPause) == nil) and not g_Combat and not GetDialog("ConversationDialog") and not GetDialog("PhotoMode") and (gv_ActivePause or not IsSetpiecePlaying())
end
function SetActivePause(enable, player_id)
  gv_ActivePause = not not enable
  local paused = IsPaused() and not not PauseReasons.ActivePause
  ...
  if gv_ActivePause then
    ...
    if player_id then
      ...
      text = player_info and T({
        718571504784,
        "PAUSED BY <player>",
        player = Untranslated(player_info.name)
      }) or nil
    end
