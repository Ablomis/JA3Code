...
function GetAPCostAndUnit(item, src_container, src_container_slot_name, dest_container, dest_container_slot_name, item_at_dest, is_reload)
  ...
  elseif IsEquipSlot(dest_container_slot_name) then
    if not (src_container == dest_container and IsEquipSlot(src_container_slot_name)) or src_container_slot_name ~= dest_container_slot_name then
      ap = ap + item:GetEquipCost()
      unit = dest_container
      action_name = T(622693158009, "Equip")
    end
  elseif item_at_dest and IsEquipSlot(src_container_slot_name) and (not (src_container == dest_container and IsEquipSlot(src_container_slot_name)) or src_container_slot_name ~= dest_container_slot_name) then
    ap = ap + item:GetEquipCost()
    unit = src_container
    action_name = T(622693158009, "Equip")
  end
...
function CanPlaceItemInInventory(item_name, amount, unit, dest_slot)
  ...
  local args = {
    item = item,
    dest_container = unit,
    dest_slot = dest_slot or "Inventory",
    sync_call = true,
    check_only = true
  }
