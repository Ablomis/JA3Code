...
function IModeCommonUnitControl:GetInteractableUnderCursor()
  ...
  if GetUIStyleGamepad() then
    ...
    MapForEach(vAround, "CObject", const.efSelectable + const.efVisible, function(obj)
      ...
      if distToObj > passSlabDistToCursor then
        return
      end
...
function IModeCommonUnitControl:UpdateInteractablesHighlight(noNew)
  ...
  if not noNew then
    ...
    if self.potential_target and self.potential_target ~= intensely_highlighted and not GetUIStyleGamepad() and not IsKindOf(intensely_highlighted, "GameDynamicSpawnObject") then
...
function HighlightAllInteractables(setOn)
  ...
  if CurrentMap == "" or IsChangingMap() or not Game then
    return
  end
...
function _ENV:ApplyCombatBarHidingAnimation(show, instantClose)
  ...
  UIHidingThread = threadFunc(function()
    ...
    ObjModified("UICombatBarShown")
    if not self or self.window_state == "destroying" then
      return
    end
    local b = self.box
    local mod = {
