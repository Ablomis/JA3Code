...
function PDAScreen:DrawChildren(clip_box)
  ...
  if drawWindows then
    ...
    UIL.DrawFrame(self.vignette_image_id, node.idDisplay.box, self.Rows, self.Columns, self:GetRow(), self:GetColumn(), self.FrameBox, not self.TileFrame, self.TransparentCenter, scaleX, scaleY, self.FlipX, self.FlipY)
  end
...
function PDAClass:Open()
  if not gv_SatelliteView then
    if IsKindOf(self, "PDASatelliteClass") then
  ...
  properties = {
    {
      ...
      editor = "bool",
      default = false,
      help = "Vertical scroll with RThumb"
    }
  },
...
function HUDMercClass:Open()
  if self.idPortrait.Image == "" then
    if self.context == "empty" then
      ...
      local templateId = IsKindOf(self.context, "Unit") and self.context.unitdatadef_id or IsKindOf(self.context, "UnitData") and self.context.class or type(self.context) == "table" and IsKindOf(self.context.template, "UnitData") and self.context.template.class
      if templateId then
