...
local lOperationChangedUpdateEvent = function(ud, previousOperation, _, prev_prof, interrupted)
...
function OnMsg.SquadStartedTravelling(squad)
  ...
  if GetAccountStorageOptionValue("PauseSquadMovement") then
    return
  end
  local player_squad = IsPlayer1Squad(squad)
  if not player_squad or SquadCantMove(squad) then
    return
  end
  ...
  for id, sector in sorted_pairs(gv_Sectors) do
    if sector.Mine and sector.mine_work_days then
      ...
      if daysStartDepleting - daysMineWorked == 1 then
        ...
        local text = T({
          407092530644,
          "Mine in <em><SectorName(sector)></em> is running dry and profits have started to decrease. They will continue to fall in the coming days and will eventually drop to <MinePercentAtDepleted()>%.",
          sector = sector
        })
