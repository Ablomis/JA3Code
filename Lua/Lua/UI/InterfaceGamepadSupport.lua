...
function GamepadUnitControl:ThreadProc()
  ...
  local hideGamePadCursor = IsSetpiecePlaying() or gv_SatelliteView or #Selection == 0 or SelectedObj and not SelectedObj:CanBeControlled() or CurrentActionCamera or combatAttackMode and self.crosshair or GetDialog("PhotoMode") and GetDialog("PhotoMode").isWorldUIHidden
...
function GamepadUnitControl:GamepadSelectionSetTarget(target)
  ...
  if not target then
    ...
    if self.window_state ~= "destroying" and self.desktop.keyboard_focus and self.desktop.keyboard_focus:IsWithin(bottomBar) then
      self:SetFocus()
    end
...
function PlayerPing()
  if g_RecentPings and g_RecentPings > lPingCooldownMax then
    return
  end
