...
function OnMsg.QuestParamChanged(questId)
  UpdateQuestBadges(questId)
  for id, times in pairs(gv_QuestRelations[questId]) do
...
local lFindTargetForQuestBadge = function(groupName, all, isRemove)
  if all then
...
function SavegameSessionDataFixups.PantagruelTCE(session_data)
  ...
  if not session_data.gvars then
    return
  end
  if not session_data.gvars.gv_Quests then
    return
  end
  if not session_data.gvars.gv_Quests.RescueBiff then
    return
  end
  if not session_data.gvars.gv_Quests.RescueBiff.TCE_SecondPantagruelChimurenga then
    return
  end
...
function SavegameSessionDataFixups.G6ForcedConflict(session_data)
  ...
  if not session_data.gvars.gv_Sectors then
    return
  end
  if not session_data.gvars.gv_Sectors.G6 then
    return
  end
...
function OnMsg.EnterSector()
  if gv_CurrentSectorId == "G6" then
    if not gv_Sectors.G6 then
      return
    end
    if not gv_Sectors.G6.conflict or gv_Sectors.G6.conflict.no_exploration_resolve then
      return
    end
...
function SavegameSessionDataFixups.Sanatorium(session_data)
  ...
  if not session_data.gvars.gv_Quests.Sanatorium then
    return
  end
  if not session_data.gvars.gv_Quests.Sanatorium.Completed then
    return
  end
...
function SavegameSessionDataFixups.BeastKillTCE(session_data)
  ...
  if not session_data.gvars.gv_Quests.Beast then
    return
  end
  if session_data.gvars.gv_Quests.Beast.TCE_RemoveConflict then
    return
  end
...
function SavegameSessionDataFixups.FaucheuxEndgame(session_data)
  ...
  if not session_data.gvars.gv_Quests["05_TakeDownFaucheux"] then
    return
  end
  if not session_data.gvars.gv_Quests["05_TakeDownFaucheux"].FaucheuxEscaped then
    return
  end
  session_data.gvars.gv_Quests["05_TakeDownFaucheux"].FaucheuxDead = false
  if not session_data.gvars.gv_Quests["06_Endgame"] then
    return
  end
