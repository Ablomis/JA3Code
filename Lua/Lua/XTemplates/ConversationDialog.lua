PlaceObj("XTemplate", {
  ...
  PlaceObj("XTemplateWindow", {
    ...
    PlaceObj("XTemplateFunc", {
      "name",
      "OnShortcut(self, shortcut, source, controller_id, ...)",
      "func",
      function(self, shortcut, source, controller_id, ...)
        if self.window_state == "destroying" then
          return
        end
        ...
        for i = 1, 6 do
          ...
          if ctrl:GetVisible() then
            ...
            if ctrlReplace and ctrlReplace:GetVisible() then
              ...
              first_idx = first_idx or replace_idx
        ...
        local dpad_map = {
          [1] = {
            top = false,
            down = indexReplacements[2],
            left = false,
            right = indexReplacements[4]
          },
          [2] = {
            top = indexReplacements[1],
            down = indexReplacements[3] or indexReplacements[4] or indexReplacements[5],
            left = false,
            right = indexReplacements[5] or indexReplacements[4]
          },
          [3] = {
            top = indexReplacements[2],
            down = indexReplacements[4] or indexReplacements[5],
            left = false,
            right = indexReplacements[6] or indexReplacements[4]
          },
          [11] = {
            top = false,
            down = indexReplacements[3],
            left = false,
            right = indexReplacements[4]
          },
          [31] = {
          ...
          [4] = {
            top = indexReplacements[3] or indexReplacements[2],
            down = indexReplacements[5],
            left = indexReplacements[1],
            right = false
          },
          [5] = {
            top = indexReplacements[4] or indexReplacements[3] or indexReplacements[2],
            down = indexReplacements[6],
            left = indexReplacements[2] or indexReplacements[1],
            right = false
          },
          [6] = {
            top = indexReplacements[5],
            down = false,
            left = indexReplacements[3] or indexReplacements[1],
            right = false
          },
          [41] = {
            top = indexReplacements[3] or indexReplacements[2],
            down = indexReplacements[6],
            left = indexReplacements[1] or indexReplacements[3],
            right = false
          },
          [61] = {
            top = indexReplacements[4],
            down = false,
            left = indexReplacements[3] or indexReplacements[1],
            right = false
          }
        ...
        if GetUIStyleGamepad() then
          ...
          if gamepadState.LeftThumb == point20 and not dpad_left and not dpad_right and not dpad_up and not dpad_down and not prev and not self.selected_phrase_idx and self.last_valid_selected_phrase_idx then
            prev = self.selected_phrase_idx
        ...
        if self.selected_phrase_idx then
          ...
          if ctrl:GetVisible() and shortcut == "+ButtonA" then
            ctrl:OnPress()
            ctrl:SetRollover(false)
            self.selected_phrase_idx = false
