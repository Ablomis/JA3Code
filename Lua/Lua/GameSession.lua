...
function OnMsg.ClassesGenerate(classes)
  table.insert(classes.XPauseLayer.properties, {
    id = "PauseReason",
    editor = "text",
    default = false
  })
...
function SetPauseLayerPause(pause, layer, keep_sounds)
  ...
  reason = GetUICampaignPauseReason(reason)
...
function CanLoadGame()
  if not ((not netInGame or NetIsHost()) and not GetDialog("ModifyWeaponDlg") and (Game or IsMainMenuMap())) or IsGameReplayRunning() or IsGameReplayRecording() or AutosaveRequest or GetGameBlockingLoadingScreen() then
    return false
  end
  return true
end
