...
function Combat:End()
  ...
  for i, s in ipairs(playerSquadsHere) do
    for _, uId in ipairs(s.units) do
      ...
      alive_player_units = not unit or alive_player_units or not unit:IsDead() and not unit:IsDowned()
...
function GetCombatPath(unit, stance, ap, end_stance)
  ...
  if g_CombatPath and unit == g_CombatPath.unit and stance == g_CombatPath.stance and (end_stance or unit.stance) == (g_CombatPath.stance_at_end or unit.stance) and g_CombatPath.start_pos == unit:GetPos() and g_CombatPath.ap == unit.ActionPoints and g_CombatPath.move_modifier == unit:GetMoveModifier(stance) and (not ap or ap == g_CombatPath.ap) then
    return g_CombatPath
  end
...
if Platform.developer then
  ...
  function randomstuff_changesector()
    ...
    local available_sectors = table.filter(gv_Sectors, function(id, sector)
      return sector.Map and id ~= gv_CurrentSectorId
    end)
  ...
  function DoRandomStuffForever()
    ...
    local randomstuff_routines = {
      {
        func = randomstuff_selection_cycle_units,
        weight = 100
      },
      {
        func = randomstuff_clickmousearound,
        weight = 100
      },
      {
        func = randomstuff_inventory,
        weight = 100
      },
      {
        func = randomstuff_modifyweapondlg,
        weight = 100
      },
      {
        func = randomstuff_satview,
        weight = 100
      },
      {
        func = randomstuff_changesector,
        weight = 50
      }
    }
    local last_routine
