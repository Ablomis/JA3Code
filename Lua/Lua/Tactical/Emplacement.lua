...
function MachineGunEmplacement:ExplorationUpdateTick()
  ...
  if not self.exploration_manned or IsValid(self.manned_by) then
    return
  end
  ...
  for _, unit in ipairs(g_Units) do
    if unit.team and unit.team.player_enemy then
      ...
      if dist <= self.personnel_search_dist and (not gunner or mindist > dist) then
  ...
  if gunner then
    ...
    if action and gunner:CanInteractWith(self) and AIStartCombatAction(action.id, gunner, 0, {target = self}) then
