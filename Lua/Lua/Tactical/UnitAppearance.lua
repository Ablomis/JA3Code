...
function Unit:AimTarget(attack_args, attack_results, prepare_to_attack)
  ...
  if not rotate_to_target and self.stance == "Prone" and attack_args.stance ~= "Prone" then
    idle_aiming = true
  elseif not rotate_to_target and aimIK and abs(self:AngleToPoint(aim_pos)) > 3000 then
  ...
  if aimIK then
    if not self.aim_rotate_cooldown_time or 0 <= GameTime() - self.aim_rotate_cooldown_time then
      self:SetIK("AimIK", aim_pos)
    end
  else
    self:SetIK("AimIK", false)
