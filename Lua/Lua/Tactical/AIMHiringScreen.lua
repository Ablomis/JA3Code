...
function TFormat.MercPriceAIMMessenger(ctx, days, include_medical)
  if not ctx then
    return
  end
...
local lPresetLevelChanges = {
  Lines = {
    {
      meta = "aimbot",
      Text = T(487770557196, "A.I.M. has increased merc salary based on their recent accomplishments. 7-Day fee is now <MercPriceAIMMessenger()>.")
    }
  }
}
...
function FindNextInputOrHireNode(merc, convoCtx, currentConvo)
  ...
  repeat
    ...
    if 50 < infiniteLoopPrevention then
      break
    end
    isHireNode = MechChatHireNode(nextNode)
  until not (not input and nextNode) or isHireNode
...
function StartMercChat(mercId)
  ...
  if canContact == "disabled" or canContact == "hidden" then
    return
  end
