...
function AttackReaction(action, attack_args, results, can_retaliate)
  ...
  can_retaliate = can_retaliate and not attack_args.stealth_attack
  ...
  for _, hit in ipairs(hits) do
    ...
    if can_retaliate and unit and g_Combat and teamPlaying ~= unit.team and HasPerk(unit, "Shatterhand") and (not results.melee_attack or not HasPerk(attacker, "HardBlow")) and not unit:HasStatusEffect("KnockDown") and not unit:HasStatusEffect("Protected") and not table.find(hit_units, unit) then
...
function CountWeaponUpgrades(weapon)
  ...
  for i, slot in ipairs(Presets.WeaponUpgradeSlot.Default) do
    ...
    if enabled and 0 < num then
      local comp = weapon.components and weapon.components[slotId]
      local def = WeaponComponents[comp]
      local upgraded = def and 0 < #(def.ModificationEffects or empty_table)
