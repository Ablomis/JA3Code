...
function RefreshPopupFocus()
  if #(g_ZuluMessagePopup or empty_table) == 0 then
    return
  end
  local desktop = terminal.desktop
  local top, topZ = false, 0
  for i, popup in ipairs(g_ZuluMessagePopup) do
    if desktop and popup:IsWithin(desktop) and (not top or topZ < popup.ZOrder or popup.ZOrder == topZ) then
      top = popup
      topZ = popup.ZOrder
    end
  end
...
function IsPausedByGameLogic()
  return IsPaused() and not IsActivePaused() and not GetDialog("PDADialogSatellite") and not gv_Deployment and not GetDialog("RadioBanterDialog") and not GetDialog("FullscreenGameDialogs")
end
...
function XActionsHost:OnShortcut(shortcut, source, ...)
  if shortcut == "+TouchPadClick" then
    shortcut = "+Back"
  elseif shortcut == "TouchPadClick" then
    shortcut = "Back"
  elseif shortcut == "-TouchPadClick" then
    shortcut = "-Back"
  end
