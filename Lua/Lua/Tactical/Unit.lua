...
function Unit:Downed()
  ...
  self:SetTargetDummy(nil, nil, nil, target_dummy_phase)
  Halt()
end
...
function Unit:Pain(hit_descr, attacker)
  ...
  if alert_units then
    TriggerUnitAlert("noise", self, const.Combat.PainNoiseRange, Presets.NoiseTypes.Default.Pain.display_name, attacker)
  end
  if self.species ~= "Human" then
    DeleteThread(self.pain_thread)
...
function Unit:CanBeControlled(player_control_mask)
  ...
  if not self:IsLocalPlayerControlled(player_control_mask) then
    return false
  end
        ...
        for _, dummy in ipairs(goto_dummies) do
          if dummy.insert_idx then
            ...
            elseif dummy.insert_before_pos then
              dummy.insert_idx = table.find(self.combat_path, dummy.insert_before_pos)
            end
          end
        end
      end
...
function Unit:UpdateMoveSpeed()
  ...
  if not g_Combat and self:IsMerc() then
    ...
    elseif self.stance == "Standing" then
...
function Unit:IsInterruptable()
  if self.interruptable and not self.goto_stance and not self.goto_hide then
    return true
  end
...
function Unit:GetBaseDamage(weapon, target, breakdown)
  ...
  if IsKindOf(weapon, "Firearm") then
    ...
    if HasPerk(self, "WeaponPersonalization") and weapon:IsFullyModified() then
      ...
      if breakdown then
        breakdown[#breakdown + 1] = {
          name = CharacterEffectDefs.WeaponPersonalization.DisplayName,
          value = baseDamageBonus
        }
      end
    end
...
function Unit:GetClosestMeleeRangePos(target, stance, interaction)
  ...
  if g_Combat then
    ...
    if positions then
      ...
      if has_path and table.find(positions, pf_closest_pos) and (not (not interaction and IsKindOf(target, "Unit")) or IsMeleeRangeTarget(self, pf_closest_pos, stance, target)) then
...
function Unit:GetBaseAimLevelRange(action, target)
  ...
  if 0 < max then
    ...
    if IsKindOf(actionWep, "Firearm") then
      ...
      local firstShotBoost = not self.performed_action_this_turn and not startedCombat and GetComponentEffectValue(actionWep, "FirstShotIncreasedAim", "min_aim")
