...
local InvalidatePindownLinesCaches = function()
...
function Unit:CanUseMeleeTraining()
  return not self:IsIncapacitated() and self:IsAware() and not self:IsDefeatedVillain() and self.stance ~= "Prone" and not self:HasStatusEffect("Protected") and self:CanActivatePerk("MeleeTraining") and (self:GetActiveWeapons("MeleeWeapon") or not not not self:GetActiveWeapons()) and not self:HasStatusEffect("BandageInCombat") and not self:HasStatusEffect("Unconscious")
end
...
function Unit:CheckProvokeOpportunityAttacks(trigger_type, target_dummies, visible_only, return_result, trigger_attack_type)
  ...
  if next(g_Overwatch) and not hardblow and (trigger_type == "attack interrupt" or trigger_type == "move" and not HasPerk(self, "LightStep")) then
    ...
    for _, data in ipairs(attackers) do
      local attacker = data.unit
      if attacker:IsOnEnemySide(self) and (g_Combat or attacker.team.player_enemy and attacker:HasStatusEffect("ManningEmplacement")) and (not visible_only or HasVisibilityTo(self.team, attacker)) and not table.find(self.passed_interrupts, 2, attacker) then
        local conditions = {}
