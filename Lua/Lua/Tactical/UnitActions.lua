...
function Unit:CombatBandage(target, medicine)
  ...
  if g_Combat then
    ...
    self:AddStatusEffect("BandageInCombat")
    while IsValid(target) and not target:IsDead() and (target.HitPoints < target.MaxHitPoints or target:HasStatusEffect("Bleeding")) and 0 < medicine.Condition do
      Sleep(5000)
      target:GetBandaged(medicine, self)
    end
...
function Unit:ResolveSteroidPunch(args, results)
  ...
  while free_slabs < pushSlabs + 1 do
    ...
    if not (nextPos and IsPassSlabStep(curPos, nextPos, const.TunnelTypeWalk)) or IsOccupiedExploration(nil, nextPos:xyz()) then
      break
    end
    toPos = curPos
    curPos = nextPos
