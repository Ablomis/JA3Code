...
DefineClass.ValuablesStack = {
  __parents = {
    "Valuables",
    "InventoryStack"
  }
}
DefineClass.ValuableItemContainer = {
  __parents = {
    "InventoryItem"
  }
}
...
DefineClass.QuestItemValuable = {
  __parents = {"QuestItem", "Valuables"}
}
...
function CashInItem(inventory, slot_name, item, amount, dontLog)
  ...
  if not dontLog then
    if amount == 1 then
      CombatLog("important", T({
        279086132540,
        "Cashed in <em><item></em> for <em><money(money_gained)></em>",
      ...
      CombatLog("important", T({
        879321398432,
        "Cashed in <em><amount> x <items></em> for <em><money(money_gained)></em>",
        amount = amount,
        items = item.DisplayNamePlural,
        money_gained = money
      }))
    end
  end
...
function UnpackItem(inventory, slot_name, item, amount)
  ...
  for i, item in ipairs(items) do
    ...
    if item.Amount and item.Amount > 1 then
      CombatLog("important", T({
        478224212958,
        "Unpacked <itemAmount> <itemDisplayName>",
...
function InvContextMenuFilter(context, className)
  ...
  if className and not IsKindOf(context.item, className) then
    return false
  end
...
function InvContextMenuEquippable(context)
  ...
  if not InvContextMenuFilter(context) then
    return false
  end
  return context.item:IsKindOfClasses("Grenade", "QuickSlotItem", "Armor") or not context.item:IsKindOfClasses("InventoryStack", "ToolItem", "Medicine", "Valuables", "QuestItem", "ConditionAndRepair", "MiscItem", "TrapDetonator", "ValuablesStack")
end
