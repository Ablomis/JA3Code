...
function OnMsg:LockpickableBrokeOpen()
  ...
  if IsKindOf(self, "Inventory") then
    self:ForEachItem(function(item, slot_name, left, top)
      ...
      if (not item.guaranteed_drop or IsKindOf(item, "QuestItem")) and IsKindOf(item, "InventoryStack") then
  ...
  for idx, merc in ipairs(all_mercs) do
    ...
    if unit then
      unit:ForEachItemDef(ItemId, function(item, slot, self_Amount)
        ...
        if not calc_all and self_Amount <= amount then
          return "break"
        end
      end, Amount)
    end
