...
function GetSaveGamesGrouped(saveObject, filter, newSave)
  ...
  local gameId = not Game or Game.testModGame and "Mod" or Game.id
  if newSave then
    ...
    if not playthrough then
      playthroughs[gameId] = {
        id = gameId,
        displayName = Game.testModGame and "Mod Tests" or save.metadata.playthrough_name,
        saves = {}
      }
      table.insert(playthroughs, playthroughs[gameId])
...
function XDesktop:OnFileDrop(filename)
  if string.ends_with(filename, ".sav", true) then
    CreateRealTimeThread(function()
      LoadGame_SkipAnySetpieces()
      if not CanLoadGame() or GetLoadingScreenDialog() then
        return
      end
      Savegame.Unmount()
...
function SavegameSessionDataFixups.DeadMercsDeleted(data, metadata, lua_ver)
  if not data.gvars.gv_UnitData then
    return
  end
  if not data.gvars.gv_Quests then
    return
  end
  local mercTrackerQuest = data.gvars.gv_Quests.MercStateTracker
  if not mercTrackerQuest then
    return
  end
  ForEachMerc(function(mId)
    if not data.gvars.gv_UnitData[mId] then
      local mercTracker = mercTrackerQuest[mId]
      if mercTracker and mercTracker.HireCount then
