...
function SectorEnableCustomOperation:__exec(obj, context)
  ...
  if self.DisplayLog then
    CombatLog("important", T({
      537039992460,
      "New <em>Sector Operation</em> available (<em><SectorId(sector)></em>): <em><ActivityName></em>",
...
function SectorEnterConflict:__exec(obj, context)
  ...
  if self.conflict_mode then
    ForceEnterConflictEffect(sector, self.spawn_mode, self.disable_travel, self.lock_conflict, self.descr_id, self.no_exploration_resolve and "force-exploration-only" or "force")
  else
