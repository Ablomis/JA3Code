...
function SetpiecePrgs.SmileyChurchFight(seed, state, TriggerUnits)
  local li = {
  ...
  local _, Pastor
  prgdbg(li, 1, 5)
  _, Pastor = sprocall(SetpieceAssignFromGroup.Exec, SetpieceAssignFromGroup, state, rand, Pastor, "Pastor", "Pastor", "Object", false)
  prgdbg(li, 1, 6)
  sprocall(SetpieceSetStance.Exec, SetpieceSetStance, state, rand, false, "", Pastor, "Crouch", "Current Weapon", true)
  local _, PastorCurchFight
  prgdbg(li, 1, 7)
  _, PastorCurchFight = sprocall(SetpieceTeleport.Exec, SetpieceTeleport, state, Pastor, "PastorCurchFight", true)
  local _, Enforcer
  prgdbg(li, 1, 8)
