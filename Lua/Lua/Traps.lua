...
function Landmine:UpdateTriggerRadiusFx(delete)
  ...
  if not (not self.done and self.visible) or range == 0 or self.TriggerType ~= "Proximity" or delete or not self:IsValidPos() then
...
local lBoobyTrapTypes = {
  {
    text = "None",
    id = const.BoobyTrapNone
  },
  {
    text = "Explosive",
    id = const.BoobyTrapExplosive,
    func = Trap.Explode
  },
  {
    text = "Electrical",
    id = const.BoobyTrapElectric,
    func = ElectricalTrap
  },
  {
    text = "Alarm",
    id = const.BoobyTrapAlarm,
    func = AlarmTrap
  }
}
...
function ExplosiveObject:TakeDirectDamage(dmg, floating, log_type, log_msg, attacker, hit_descr)
  ...
  if inIdle and not self.explodePart and 20 < dmg then
