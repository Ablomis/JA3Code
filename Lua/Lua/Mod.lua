...
function CheatNewModGame(start_type)
  CreateRealTimeThread(function()
    if start_type == "quickstart" then
      if WaitQuestion(terminal.desktop, Untranslated("Quick Start"), Untranslated([[
A new quick test mod game will be started. It will skip the merc hire & arrival phase.
...
A new test mod game will be started.
...
function IsInModTestingMode()
  return not IsModEditorMap(CurrentMap) and Game and Game.testModGame
end
...
local UpdateAppearanceOnSpawnedObj = function(id)
  ...
  properties = {
    {
      ...
      editor = "help",
      help = Untranslated([[
The <em>Translated Voices</em> mod item allows to supply to the game voices which will be used when the game is running in a specific language. You can use this to add new language voices of the existing voice lines, to override the existing voice lines, or to add entirely new voice content to the game.
...
3. To supply voices in multiple languages, use one <em>Translated Voices</em> mod item per language.]])
    },
    {
      ...
      editor = "dropdownlist",
      default = "",
      help = Untranslated("Based on this value, the mod will decide if it should load the voices located in the translation folder."),
      items = GetAllLanguages()
    },
    {
      id = "translatedVoicesFolder",
      name = "Translated Voices Folder",
      editor = "browse",
      filter = "folder",
      default = "",
      help = Untranslated("The folder inside the mod in which the translated voices should be placed. If you use the Convert & Import mod item for creating the files, pick the same path as the one defined there.")
    },
    {
      id = "btn",
      editor = "buttons",
...
function ModItemTranslatedVoices:TryMountFolder()
  if self.translatedVoicesFolder ~= "" and (GetLanguage() == self.language or self.language == "Any") then
    local err = MountFolder("CurrentLanguage/Voices", self.translatedVoicesFolder, "seethrough,label:" .. self.mod.id)
    if err then
