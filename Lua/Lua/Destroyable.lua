...
function ShouldDestroyObject(obj, clsTable)
  ...
  if obj:GetGameFlags(const.gofPermanent) ~= 0 and not obj:GetParent() and not IsKindOfClasses(obj, table.unpack(clsTable)) and (not IsKindOf(obj, "DestroyableWallDecoration") or not obj.managed_by_slab) then
    return true
  end
  return false
end
...
AppendClass.TwoPointsAttachParent = {children = false}
...
local RemoveInvalidDestroyedObjects = function(objects)
