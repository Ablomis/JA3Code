...
function TFormat.ItemLog(itemLog, unit, isSingleEntry)
  ...
  elseif unit then
    if IsKindOf(unit, "SectorStash") then
      ...
      res = T({
        581384045758,
        " <amount> x <em><itemNameT></em> (<mercName>)",
        amount = amount,
        itemNameT = itemNameT,
        mercName = unit:GetDisplayName()
      })
    ...
    res = T({
      437609056132,
      " <amount> x <em><itemNameT></em> (squad bag)",
      amount = amount,
      itemNameT = itemNameT
    })
  end
...
function MercPresetCombo()
  local ret = {}
  ForEachMerc(function(preset)
    table.insert(ret, preset)
  end)
  table.sort(ret)
  return ret
end
